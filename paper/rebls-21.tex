...
delimited sections to initialize memory \lin{4--8}, read inputs in a loop
...
This section of code is expanded in Figure~\ref{fig.input} and is discussed in
sequence.
...
Figure~\ref{fig.input} expands the output section with the calls to
\texttt{gals\_emit} to generate asynchronous inputs.
In this application, it simply calls the SDL library to check if a key is
pressed and forward to the middleware.
Note that in a local-only application this section of code would replace the
section to read inputs from the middleware.
