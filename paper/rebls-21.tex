...
deterministically.
Hence, we consider determinism to be a property of individual instances, while
consistency to be a property of the whole distributed system.
...
%
Even though deterministic distributed timelines are impossible, our goal is to
guarantee consistently timelines.
%
More specifically, we propose to extend Lamport's sequential consistency
model~\cite{lamport} with third property for timing accuracy as follows:
%
\begin{enumerate}
...
\end{enumerate}
%
We name it \emph{timely-sequential consistency} due to the third property.
...
As highlighted in the previous paragraph, outputs are actually asynchronous
...
identically.
Hence, the middleware satisfies the timely-sequential consistency model for
...
event \emph{1} towards the server at local time \emph{750ms}
(\emph{async(750,1)}).
%
...
\emph{goal~3}.
As an additional remark, note that pausing \emph{Client~1} increased the offset
difference even further (\emph{offset diff}).
%
...
We simulated a variety of scenarios by parameterizing the following factors:
%
\begin{itemize}
...
In our proposal, we introduce a middleware to tame nondeterminism and provide
global sequential consistency to all nodes.
%
