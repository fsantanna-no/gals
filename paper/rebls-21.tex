...
\end{enumerate}
%
For instance, for a video game with a few friends over Wi-Fi, the parameters
would be
    \emph{3--5} instances,
    an RTT of \emph{5--10ms},       (TODO)
    FPS around \emph{50},
    and a new interaction around every \emph{500ms}.
An interactive remote class over the internet could reach
    \emph{100} instances,
    an RTT of \emph{200ms},         (TODO)
...
%
\begin{itemize}
\item \emph{N}: $2$, $5$, $10$, and $100$ instances.
\item \emph{NET}: \emph{localhost}, \emph{local/Wi-Fi}, and
                  \emph{external/Internet} types of network (with RTTs in the
                  range of \emph{1--100ms}.  (TODO)
\item \emph{FPS}: $10$, $20$, $40$, $50$, $100$ frames per second.
\item \emph{EVT}: $10s$, $5s$, $1s$, and $100ms$ period between each event.
\end{itemize}
%
This values combined result in $240$ combinations.
Each combination executes for $10$ minutes and is repeated $5$ times.
The experiments take $200$ hours to complete.
